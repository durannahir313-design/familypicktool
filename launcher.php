...
/**
 * ==========================================================
 *  UsdPick BOT v3.0 — Standalone Edition
 *  Extraido de MULTI-PICK BOT v3.0 por @Oprahuang
 *  Licencia SHA-256 + Hardware ID Binding
 * ==========================================================
 *  Sitio : UsdPick (usdpick.io)
 *  Crypto: USDT (USDTUSDT)
 *  Nota  : Usa NextAuth v5 + Turnstile (sitekey fijo)
 * ==========================================================
 */
...
function licenseGate(){
    ...
    if($cache){
        if(in_array($cache['hash'],$lic['hashes'])){
            ...
            // Si no hay HWID en lic.dat (campo vacio) y hay cache, dejar pasar pero pedir reactivacion
    ...
    // Verificar HWID desde license.dat (admin-controlled)
    $bound=isset($lic['hwids'][$hash])?$lic['hwids'][$hash]:'';
    if($bound===''){
        // Sin HWID asignado — activacion pendiente
    ...
    if($bound!==$myShort){
        // HWID NO COINCIDE — dispositivo no autorizado
        echo NL.C_RED."+============================================+".C_RESET.NL;
    ...
    // HWID COINCIDE — acceso permitido
...
function showBanner($siteName,$crypto){
    ...
    $sep=str_repeat("-",$W);
    echo NL.$cy.$sep.$r.NL;
    echo $cy."|".C_RED.ui_center("MULTI-PICK BOT v3.0",$W).$cy."|".$r.NL;
...
function getCryptoPrice($pair){
    if($pair==='USDTUSDT')return 1.0; // USDT siempre vale $1
...
define('USDPICK_TURNSTILE_SITEKEY','0x4AAAAAAAxf0PSg12ZfIp7o');

// =================== SOLVE CAPTCHA (MAIN) ===================
function solveCaptcha($html,$host,$cookieStr,$ua,$pageUrl){
    // === SuiPick/UsdPick: Skip IconCaptcha, usar Turnstile via API directamente ===
    // El bypass IconCaptcha gratuito es detectado y rechazado por estos sitios.
    // Se usa Turnstile via Xevil/Multibot API (igual que suiv1.php que funciona).
    if($host==='suipick.io'){
        echo C_CYAN."[*] ".C_WHITE."SuiPick: Resolviendo Turnstile via API (sitekey fijo)...".NL;
        $cap=captchaSolve($pageUrl,SUIPICK_TURNSTILE_SITEKEY,"turnstile");
        if($cap){
            return[
                'type'=>4,
                'captcha'=>"c_captcha_response=".urlencode($cap),
                'suipick_simple'=>true // flag para formato simple
            ];
        }
        echo C_RED."[!] SuiPick: Fallo Turnstile via API, intentando deteccion automatica...".NL;
        // Fallback a deteccion automatica si la API falla
    }
    // UsdPick usa NextAuth - sus funciones (usdpickLogin/usdpickFaucet) resuelven captcha
    // directamente via captchaSolve() con sitekey fijo. No necesita pasar por solveCaptcha().
    // Si llega aqui, es un fallback para deteccion automatica.

    // 1) Intentar IconCaptcha (GRATIS) — solo para sitios que no sean suipick.io
    if(strpos($html,"_iconcaptcha-token")!==false){
        ...
        if($cap){
            ...
            return $cap; // ya retorna ['type'=>0, 'captcha'=>string_completo]
        }
        echo C_YELLOW."[!] IconCaptcha fallo, fallback a API...".NL;
    }
    // 2) Fallback: Turnstile via API
    // Buscar sitekey turnstile en el HTML
    ...
    // NINGUN captcha detectado - mostrar debug
...
// =================== PARSE MULTI-JSON ===================
// Algunos servidores devuelven multiples JSON concatenados:
//   {"ret":0,"mes":"error"}{"ret":1,"reward":1000,...}
// json_decode falla con eso. Esta funcion los separa y retorna
// el primero con ret==1 (exito), o el ultimo parseable.
...
define('USDPICK_VERSION_HEADER','X-Version-Request: 0518998');

function usdpickRealBal($wei){return $wei/1000000000000000000;}

// Headers para UsdPick API (cookie-based auth, sin Bearer)
function usdpickHeaders($host,$ua){
    return["User-Agent: ".$ua,USDPICK_VERSION_HEADER,"authorization: ",
        "Accept: application/json","Content-Type: application/json",
        "Origin: https://{$host}","Referer: https://{$host}/"];
}

// Headers para GET requests (sin Content-Type)
function usdpickHeadersGet($host,$ua){
    return["User-Agent: ".$ua,USDPICK_VERSION_HEADER,"authorization: ",
        "Accept: application/json",
        "Origin: https://{$host}","Referer: https://{$host}/"];
}

// Obtener balance real de /api/account/tokens/usdt
function usdpickGetBalance($site){
    $host=$site['host'];
    $ua=saveData($host,'user_Agent');
    $h=usdpickHeadersGet($host,$ua);

    // GET /api/account/tokens/usdt → {data: {id, balance (wei), ...}}
    $r=Run("https://{$host}/api/account/tokens/usdt",$h);
    if(!$r)return['user'=>'','balance'=>'0.00000000','balance_wei'=>0];
    $data=safe_json_decode($r);
    if(!$data)return['user'=>'','balance'=>'0.00000000','balance_wei'=>0];

    $balWei=0;
    if(isset($data['data']['balance']))$balWei=(float)$data['data']['balance'];
    $balReal=number_format(usdpickRealBal($balWei),8);

    // Obtener user de /api/auth/session (solo si es necesario)
    $user='';
    $rs=Run("https://{$host}/api/auth/session",$h);
    if($rs){
        $sd=safe_json_decode($rs);
        if($sd&&isset($sd['user']))$user=$sd['user']['name']??($sd['user']['email']??'');
    }

    return['user'=>$user,'balance'=>$balReal,'balance_wei'=>$balWei];
}

// Obtener estado del faucet (timer)
function usdpickGetFaucetStatus($site){
    $host=$site['host'];
    $ua=saveData($host,'user_Agent');
    $h=usdpickHeadersGet($host,$ua);
    $r=Run("https://{$host}/api/faucet",$h);
    if(!$r)return null;
    return safe_json_decode($r);
}

// Calcular segundos restantes hasta cycle_ended_at
function usdpickTimer($cycleEndedAt){
    date_default_timezone_set('UTC');
    $end=new DateTime($cycleEndedAt);
    $now=new DateTime("now");
    $rem=$end->getTimestamp()-$now->getTimestamp();
    return max(0,$rem);
}

function usdpickLogin($site){
    $host=$site['host'];$crypto=$site['crypto'];
    // Borrar cookies viejas
    $cookieFile=__DIR__."/configs/{$host}-config/cookie.txt";
    if(file_exists($cookieFile))@unlink($cookieFile);
    $ua=saveData($host,'user_Agent');
    $email=saveData($host,'email');$pass=saveData($host,'pass');

    // 1) Obtener CSRF token de Auth.js
    echo C_CYAN."[*] UsdPick: Obteniendo CSRF token...".NL;
...
".C_RESET;sleep(3);return usdpickLogin($site);}
    ...
    // 3) Login via POST /api/auth/signin con JSON
    echo C_CYAN."[*] UsdPick: Enviando login...".NL;
    $loginUrl="https://{$host}/api/auth/signin";
    $loginPayload=['email'=>$email,'password'=>$pass,'captcha_token'=>$captchaToken];
    if($csrfToken!=='')$loginPayload['csrfToken']=$csrfToken;
    $postData=json_encode($loginPayload);
...
".C_RESET;sleep(3);return usdpickLogin($site);}
    ...
    echo C_DIM."     [Login] HTTP {$httpCode} | Body len: ".strlen($body).C_RESET.NL;

    // 4) Verificar login exitoso obteniendo la sesion
    sleep(2);
...
".C_RESET;return false;}
    ...
    if($sessionData&&isset($sessionData['user'])){
        $userName=$sessionData['user']['name']??($sessionData['user']['email']??$email);
        rewardBox("Login",["INFO"=>"Login Successfully","Host"=>$host,"Account"=>$userName]);
        return true;
    }

    // Si no hay sesion, verificar errores
    $loginData=safe_json_decode($body);
    $errorMsg='Error desconocido';
    if($loginData&&isset($loginData['error']))$errorMsg=is_string($loginData['error'])?$loginData['error']:json_encode($loginData['error']);
...
function usdpickDashboard($site){
    $d=usdpickGetBalance($site);
    if(!$d['user']){
        echo C_YELLOW."[!] Sesion expirada, re-login...\n".C_RESET;
        usdpickLogin($site);
        $d=usdpickGetBalance($site);
    }
    $price=1.0; // USDT = 1 USD
...
function usdpickFaucet($site){
    global $CLAIM_COUNT;$host=$site['host'];$crypto=$site['crypto'];
    static $usdpickFailCount=0;
    while(true){
        $ua=saveData($host,'user_Agent');

        // 1) Verificar estado del faucet (GET /api/faucet)
        $status=usdpickGetFaucetStatus($site);
        if($status&&isset($status['data']['cycle_ended_at'])){
            $timer=usdpickTimer($status['data']['cycle_ended_at']);
            if($timer>0){
                echo C_YELLOW."[*] UsdPick: Faucet en cooldown, esperando...".C_RESET.NL;
                countdownDisplay($timer,"Siguiente reclamo en");
            }
        }

        // 2) Resolver Turnstile captcha
        echo C_CYAN."[*] Resolviendo captcha para claim #".($CLAIM_COUNT+1)."...".NL;
        $cap=captchaSolve("https://{$host}/faucet",USDPICK_TURNSTILE_SITEKEY,"turnstile");
        if(!$cap){echo C_RED."[!] Fallo captcha, reintentando en 60s...\n".C_RESET;sleep(60);continue;}

        // 3) Enviar claim a POST /api/faucet
        $captchaToken="turnstile:".$cap;
        echo C_CYAN."[*] UsdPick: Enviando claim...".NL;
        $h=usdpickHeaders($host,$ua);
        $h[]="Referer: https://{$host}/faucet";
        $postData=json_encode(['captcha_token'=>$captchaToken]);

        $raw=Run1("https://{$host}/api/faucet",$h,$postData);
        if(!$raw){
            $usdpickFailCount++;
            echo C_RED."[!] Sin respuesta del servidor (intento #{$usdpickFailCount}).\n".C_RESET;
            if($usdpickFailCount>=3){usdpickLogin($site);$usdpickFailCount=0;}
            sleep(30);continue;
        }

        $claimBody=$raw['body'];
        $httpCode=$raw['info']['http_code']??0;
        $claim=safe_json_decode($claimBody);

        // Debug: mostrar respuesta
        echo C_DIM."     [UsdPick] HTTP {$httpCode} | Response: ".substr(strip_tags($claimBody),0,250).C_RESET.NL;

        // Si no es JSON valido
        if(!is_array($claim)){
            $usdpickFailCount++;
            echo C_RED."[!] Respuesta invalida del servidor (intento #{$usdpickFailCount}).\n".C_RESET;
            echo C_DIM."     HTTP Code: {$httpCode} | Body len: ".strlen($claimBody).C_RESET.NL;
            if($usdpickFailCount>=3){
                echo C_YELLOW."[UsdPick] Demasiados fallos, re-logueando...".C_RESET.NL;
                $cookieFile=__DIR__."/configs/{$host}-config/cookie.txt";
                if(file_exists($cookieFile))@unlink($cookieFile);
                usdpickLogin($site);
                $usdpickFailCount=0;
        ...
        // Tenemos JSON! Resetear contador
        $usdpickFailCount=0;

        // Manejar captcha invalido
        if(isset($claim['error'])&&stripos($claim['error'],"captcha")!==false){
            echo C_YELLOW."[!] Captcha rechazado: ".$claim['error'].C_RESET.NL;
            sleep(10);continue;
        }

        // Manejar sesion expirada
        if(isset($claim['error'])&&(
            stripos($claim['error'],"session")!==false||
            stripos($claim['error'],"unauthorized")!==false||
            stripos($claim['error'],"not found")!==false||
            stripos($claim['error'],"login")!==false||
            stripos($claim['error'],"auth")!==false
        )){
            echo C_YELLOW."[!] Sesion expirada, re-login...\n".C_RESET;
            usdpickLogin($site);
            sleep(5);continue;
        ...
        // Manejar rate limit
        if(isset($claim['error'])&&(
            stripos($claim['error'],"try again")!==false||
            stripos($claim['error'],"rate limit")!==false||
            stripos($claim['error'],"too many")!==false||
            stripos($claim['error'],"already claimed")!==false||
            stripos($claim['error'],"wait")!==false
        )){
            // Intentar obtener timer del status
            $waitSec=3600;
            $status2=usdpickGetFaucetStatus($site);
            if($status2&&isset($status2['data']['cycle_ended_at'])){
                $t=usdpickTimer($status2['data']['cycle_ended_at']);
                if($t>0)$waitSec=$t+60; // +60s margen
            }
            echo C_YELLOW."[UsdPick] Rate limit, esperando...".C_RESET.NL;
            countdownDisplay($waitSec,"Rate limit, esperando");
            continue;
        }

        // Otro error
        if(isset($claim['error'])){
            echo C_RED."[!] Error: ".$claim['error'].NL;
            echo C_DIM."     [UsdPick Debug] ".substr(json_encode($claim),0,300).C_RESET.NL;
            sleep(30);continue;
        }

        // Verificar exito - claim exitoso tiene data.claimed_amount
        $success=false;
        if(isset($claim['data'])&&is_array($claim['data']))$success=true;
        if(isset($claim['success'])&&$claim['success'])$success=true;

        if(!$success){
            echo C_RED."[!] Respuesta inesperada: ".substr(json_encode($claim),0,300).NL;
            sleep(30);continue;
        }

        // EXITO! Mostrar reward
        $CLAIM_COUNT++;
        $claimedWei=isset($claim['data']['claimed_amount'])?$claim['data']['claimed_amount']:0;
        $reward=number_format(usdpickRealBal((float)$claimedWei),8);

        // Obtener balance actualizado
        $balInfo=usdpickGetBalance($site);
        $bal=$balInfo['balance'];

        $mes="You got {$reward} {$crypto}!";
        if(isset($claim['data']['message']))$mes=$claim['data']['message'];
        $price=1.0;$usd=number_format((float)$bal*$price,4);
        faucetReward([""=>$mes,"Balance"=>$bal." {$crypto} (\${$usd} USDT)","Claim #"=>"[".$CLAIM_COUNT."]","Captcha"=>"Turnstile"]);

        // Esperar usando cycle_ended_at del claim response
        if(isset($claim['data']['cycle_ended_at'])){
            $timer=usdpickTimer($claim['data']['cycle_ended_at']);
            if($timer>0){
                countdownDisplay($timer+60,"Siguiente reclamo en"); // +60s margen
            }else{
                countdownDisplay(3605,"Siguiente reclamo en");
            }
        }else{
            countdownDisplay(3605,"Siguiente reclamo en");
        }
...
// =================== SITE CONFIG ===================
$SITE = ['name'=>'UsdPick', 'host'=>'usdpick.io', 'crypto'=>'USDT', 'pair'=>'USDTUSDT'];
$API_TYPE = 0;
$API_HOST = '';
$API_KEY  = '';
$CURRENT_SITE = null;
...
// Cargar o configurar API
if(!loadApi()){selectApi();}

showBanner($SITE['name']." -- ".$SITE['host'],$SITE['crypto']);
setupSite($SITE);

echo C_CYAN."[*] Iniciando sesion en {$SITE['name']} (NextAuth)...".NL;
$ok=usdpickLogin($SITE);
if(!$ok){echo C_RED."[!] No se pudo iniciar sesion.
".C_RESET;sleep(3);exit(1);}
usdpickDashboard($SITE);
...
";
    ...
    if($opt==='1'){usdpickFaucet($SITE);}
    elseif($opt==='2'){usdpickDashboard($SITE);}
    elseif($opt==='3'){exit(C_YELLOW."
[*] Cerrando bot...
".C_RESET);}
}
